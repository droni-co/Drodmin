<template>
  <div class="flex py-3">
    <div class="text-sm" v-if="!compact">
      ...
      <input type="number" v-model="nperPage" max="100" min="10" step="1" v-on:keyup.enter="$emit('paginate', {npage: props.page, nperPage: nperPage})" class="w-16 px-2 py-1 rounded border border-gray-300 text-center">
    </div>
      ...
      <nav>
        <template v-for="npage in Math.ceil(total / perPage)" :key="npage">
          ...
          <div v-if="
            ...
            class="cursor-pointer inline px-2 py-1 rounded hover:bg-gray-200"
